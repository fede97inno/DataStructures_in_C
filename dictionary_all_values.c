...
void aiv_dict_insert(struct aiv_dict* dict, const char* key,const void* value , size_t value_size)
{
    ...
    for (size_t i = 0; i < HASHMAP_SIZE_LIST; i++)
    {
        if (dict->hashmap[index][i].key_len == 0)
        {
            ...
            memcpy(dict->hashmap[index][i].value, value, value_size);
            dict->hashmap[index][i].key_len = key_len;
            printf("Added (%s, value) at index %llu slot %llu\n", key, index, i);
            return;
        }
    }
